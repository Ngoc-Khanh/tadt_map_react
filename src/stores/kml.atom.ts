...
// Base atom để lưu trữ danh sáchs file KML files
...
export const addKMLFileAtom = atom(null, async (get, set, file: File) => {
  ...
  try {
    ...
    set(kmlFilesAtom, (prev) => prev.map((f) => (f.id === fileId ? { ...f, status: "parsing" } : f)));
    // Parse the file
    const parsedData = await KMLParser.parseFile(file);
    if (!parsedData || !KMLParser.validateParsedData(parsedData)) throw new Error("Invalid KML/KMZ file or no valid geometries found");
    // Get statistics
    const statistics = KMLParser.getStatistics(parsedData);
    // Update with success
    set(kmlFilesAtom, (prev) =>
      prev.map((f) =>
        f.id === fileId
          ? { ...f, status: "success", data: parsedData, statistics }
          : f
      )
    );
  } catch (error) {
